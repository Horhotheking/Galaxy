...
for (let i = 0; i < item_desc.length; i += 1) {
    item_name_objects[i].innerHTML = item_names[i]
    merch_item_descr[i].innerHTML = item_desc[i]
...
for(let i = 0; i < add_to_cart_btn.length; i += 1){
    add_to_cart_btn[i].addEventListener('click', function(){
        count++
        cart_amount.innerHTML = count
        if(merch_item_price[i].innerHTML == 1000){
            result += item_price[i]
        } else if(merch_item_price[i].innerHTML == 2500){
            result += item_price[i]   
        } else if(merch_item_price[i].innerHTML == 5000){
            result += item_price[i]
        }
        cart_text.innerHTML = result
    })
}

cart_text.addEventListener('click', function(){
    if(cart_text.innerHTML == 'Корзина'){
        cart_text.innerHTML = result
    }
    else{
